...
trait WithStateSets extends Initializable {
  ...
  private[mpmens] val _allStates = mutable.ListBuffer.empty[State]
  ...
  private[mpmens] var _stateNextId = 0
  private[mpmens] def _genStateId = {
    val result = _stateNextId
    _stateNextId = _stateNextId + 1
    result
  }

  private[mpmens] var _rootState: StateSet[State] = _
  def rootStates: StateSet[State] = _rootState

  private[mpmens] var _states: StateSetEquiv[State] = _
