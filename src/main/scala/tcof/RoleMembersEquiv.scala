...
/**
  ...
  */
class RoleMembersEquiv[+ComponentType <: Component](val linkedMembers: Iterable[LinkedMember[ComponentType]]) extends RoleMembers(linkedMembers.map(_.value)) {
  ...
  def selectEquiv[RoleType <: Component : ClassTag]: RoleMembersEquiv[RoleType] =
    new RoleMembersEquiv(linkedMembers.collect{ case member@LinkedMember(value: RoleType, parent, indexInParent) => LinkedMember(value.asInstanceOf[RoleType], parent, indexInParent) })
